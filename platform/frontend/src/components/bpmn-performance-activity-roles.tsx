...
interface BpmnPerformanceActivityRolesProps {
  activityName: string
...
export function BpmnPerformanceActivityRoles({ activityName, roles, average_time, median_time, min_time, max_time, onClose }: BpmnPerformanceActivityRolesProps) {
  ...
  return (
    ...
    >
      <div 
        className="bg-white border-1 border-[#808080] rounded-lg shadow-xl w-[50%] max-w-[800px] max-h-[80vh] flex flex-col"
        ...
        <div className='mb-4 px-4 flex flex-col flex-1 min-h-0'>
          ...
          {average_time !== undefined && (
            <div className="flex items-center gap-2 mb-3">
              <span className="text-sm font-medium text-zinc-700">{t('activityTooltip.averageTime')}</span>
              <span className="text-xs text-red-600 font-medium bg-red-100 px-2 py-0.5 rounded whitespace-nowrap">
                {formatSecondsFull(average_time, false)}
              </span>
            </div>
          )}
          {median_time !== undefined && (
            <div className="flex items-center gap-2 mb-3">
              <span className="text-sm font-medium text-zinc-700">{t('activityTooltip.medianTime')}</span>
              <span className="text-xs text-red-600 font-medium bg-red-100 px-2 py-0.5 rounded whitespace-nowrap">
                {formatSecondsFull(median_time, false)}
              </span>
            </div>
          )}
          {min_time !== undefined && (
            <div className="flex items-center gap-2 mb-3">
              <span className="text-sm font-medium text-zinc-700">{t('activityTooltip.minTime')}</span>
              <span className="text-xs text-red-600 font-medium bg-red-100 px-2 py-0.5 rounded whitespace-nowrap">
                {formatSecondsFull(min_time, false)}
              </span>
            </div>
          )}
          {max_time !== undefined && (
            <div className="flex items-center gap-2 mb-3">
              <span className="text-sm font-medium text-zinc-700">{t('activityTooltip.maxTime')}</span>
              <span className="text-xs text-red-600 font-medium bg-red-100 px-2 py-0.5 rounded whitespace-nowrap">
                {formatSecondsFull(max_time, false)}
              </span>
            </div>
          )}
